...
class GroceryList extends React.Component {
  ...
  addGroceryItem() {
    // Put your code here
    let newName = this.state.newGroceryName
    if (newName.length === 0) {
      return
    }
    let groceries = this.state.groceries.concat({name: newName})
    let _input = React.findDOMNode(this._input)
    _input.value = ""
  ...
  render() {
    ...
    newProductInput =
      <input className='new-item' ref={it => this._input = it} type="text" onChange={this.inputChanged}/>;
