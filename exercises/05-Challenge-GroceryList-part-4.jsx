...
class GroceryList extends React.Component {
  ...
  addGroceryItem(newGroceryName) {
    ...
    let newGroceryItem = {name: newGroceryName};
    this.setState({
  ...
  render() {
    ...
    return (
      <div>
        ...
        <GroceryForm addGroceryItem={this.addGroceryItem} clearGrocery={this.clearList}/>
      </div>
    );
  }
}
...
class GroceryForm extends React.Component {
  ...
  onTextChange(e) {
    // this.state.newGroceryName = e.target.value
    // if (this.state.newGroceryName) {
    //   React.findDOMNode(this._addBtn).disable = false
    // } else {
    //   React.findDOMNode(this._addBtn).disable = true
    // }
    this.setState({newGroceryName: e.target.value})
  }

  onAddGrocery() {
    this.props.addGroceryItem(this.state.newGroceryName)
  }

  onClearGrocery() {
    this.props.clearGrocery()
  }

  render() {
    let btn = <button className='add-product' ref={it => this._addBtn = it} onClick={this.onAddGrocery}
                      disabled="disabled">Add new Product </button>
    if (this.state.newGroceryName) {
      btn = <button className='add-product' ref={it => this._addBtn = it} onClick={this.onAddGrocery}>Add new
        Product </button>
    }
    return (
      <div>
        <input className='new-item' type="text" onChange={this.onTextChange}/>
        {btn}
        <button className='clear-list' onClick={this.onClearGrocery}>Clear the List</button>
      </div>
    )
  }
}
